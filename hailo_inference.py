...
class HailoAsyncInference:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            with self.infer_model.configure() as configured_infer_model:
                while True:
                    ...
                    if self.send_original_frame:
                        original_batch, preprocessed_batch, data_obj = batch_data
                    else:
            ...
            # Thread inference mà chết âm thầm thì server chạy mù (stream không detect).
            # Fail-fast để supervisor (systemd / vòng lặp .bat) restart cả process.
            logger.exception("Hailo inference thread died — exiting so the supervisor restarts the process")
            os._exit(1)
    ...
    def _create_bindings(self, configured_infer_model) -> object:
        ...
        #for name in self.output_type:
        #    print(f"Set output layer '{name}' to shape '{self.infer_model.output(name).shape}'")
